...
2차 프로젝트를 진행하면서, ESTCamp측은 최신 코드 어시스트 방법론(codex, claude code)을 활용할 때 팀당 최대 $30까지 지원해주었지만,<br>
본 팀은 해당 비용 지원을 활용하지 못했고, 대신 GPT-5 기반 LLM-as-a-Judge 평가 및 Colab 세션 추가 등에 자체 비용을 사용하였다.
> 원격 SSH 서버가 팀원 간 공유되는 A100 MIG 슬라이스 환경이었던 점도 에이전틱 코딩 도구(Codex, Claude Code) 도입을 망설이게 한 요인 중 하나였다.<br>
> 다만 GPT-5 judge 비용과 Colab 세션 비용은 지원 대상에 포함되지 않아, 향후 프로젝트에서는 지원 가능한 항목을 사전에 명확히 확인하고<br>
> 에이전틱 코딩 도구를 적극 도입하여 반복적인 디버깅(예: AutoProcessor→AutoTokenizer 스왑, 캐시 키 불일치 등) 시간을 줄이고,<br>
> 절약된 리소스를 모델별 독립 최적화나 평가 구조 개선과 같은 핵심 실험에 더 투입하는 방향을 고려할 필요가 있다.
