...
> 원격 SSH 서버가 팀원 간 공유되는 A100 MIG 슬라이스 환경이었던 점도<br>
> 에이전틱 코딩 도구(Codex, Claude Code) 도입을 망설이게 한 요인 중 하나였다.<br>
> 다음 프로젝트에서는 에이전틱 코딩 도구를 적극 도입하여<br>
> 반복적인 디버깅(예: AutoProcessor→AutoTokenizer 스왑, 캐시 키 불일치 등) 시간을 줄이고,<br>
